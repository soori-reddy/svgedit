 ...
 * @class IdentityText
 */
export class IdentityText extends HTMLElement {
  /**
  ...
  createTemplate (imgPath) {
    ...
    .menu {
      ...
      left: 20px;
      min-height: 350px;
      max-height: 550px; /* Instead of fixed height, allows auto-resizing */
        ...
        try {
          ...
          const responseData =  {
            "data": {
              "data": [
                ...
                {
                  "prop": "Employee Type",
                  "label": "${(identity.employeeType)}"
                },
                {
                  "prop": "validFrom",
                  "label": "${(identity.asset.validFrom)}"
                },
                {
                  "prop": "validTo",
                  "label": "${(identity.asset.validTo)}"
                }
              ]
            }
          }
          if(responseData && responseData.data && responseData.data.data) {
            this.$menu.innerHTML = responseData.data.data.map((menu, i) => (
              `<div data-menu="${menu.prop}" class="menu-item" title="${menu.prop}">${menu.label}</div>`
              )).join('')
          }
        } catch (error) {
          console.error(error)
...
// Register
customElements.define('se-identitytext', IdentityText)
